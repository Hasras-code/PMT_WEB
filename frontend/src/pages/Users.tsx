    ...
    <div className="space-y-6">
      {canManage && <Card className="p-7">
        ...
        <form onSubmit={searchCandidates} className="mt-4 flex gap-3">
          <input required minLength={2} aria-label="Search by student number, name, or email" placeholder="Search by student number, name, or email" className={inputCls} value={query} onChange={(e) => setQuery(e.target.value)} />
        ...
        {loading ? (
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="w-full text-left text-[15px]">
              <caption className="sr-only">Cohort members and their roles</caption>
              <thead>
                <tr className="text-muted text-sm border-b border-line">
                  ...
                  <th scope="col" className="py-3 pr-4 font-medium">Roles</th>
                  <th scope="col" className="py-3 pr-4 font-medium">Joined</th>
                  {canAssignRoles && <th scope="col" className="py-3 pr-4 font-medium">Assign role</th>}
                  {canManage && <th scope="col" className="py-3 font-medium">Lifecycle</th>}
                </tr>
              </thead>
                  ...
                  <tr key={m.id}>
                        ...
                        {m.roles.map((r) => (
                          <span key={r} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-primary-light text-primary">
                            ...
                            {canAssignRoles && r !== 'STUDENT' && canChangeRole(r) && (
                              <button onClick={() => setRole(m.id, r, true)} className="hover:text-red-600" title={`Remove ${r}`} aria-label={`Remove ${r} role from ${m.display_name}`}>
                    ...
                    {canAssignRoles && <td className="py-3.5 pr-4">
                      <div className="flex gap-2">
                        <select
                          className="px-3 py-1.5 rounded-lg border border-line bg-charcoal-card text-sm text-ink"
                          aria-label={`Assign role to ${m.display_name}`}
                          value={assign[m.id] || ''}
