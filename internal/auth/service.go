...
func (s *Service) Register(ctx context.Context, in RegisterInput) error {
	...
	e = db.Tx(ctx, s.Pool, func(tx pgx.Tx) error {
		var batchID string
		if e := tx.QueryRow(ctx, `SELECT id FROM batches WHERE id=$1 AND status='ACTIVE' FOR SHARE`, in.BatchID).Scan(&batchID); e != nil {
		...
		var id string
		e := tx.QueryRow(ctx, `INSERT INTO users(student_number,combination,registration_batch_id,first_name,last_name,display_name,email,password_hash) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT DO NOTHING RETURNING id`, in.StudentNumber, in.Combination, batchID, in.FirstName, in.LastName, in.DisplayName, in.Email, hash).Scan(&id)
		if errors.Is(e, pgx.ErrNoRows) {
